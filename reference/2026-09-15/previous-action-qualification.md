# Previous Action candidate qualification

## Meaning

This is the public technical edition of the September 14 Action candidate
...
Current proof is in [Qualification](../../docs/QUALIFICATION.md).

## Observed environment

The September 14, 2026 Action/release-candidate cut was exercised on **Linux x64**
...
| --- | --- | --- |
| Preparation assets | `npm run check:prep` | 46 JSON assets and 189 declared cases validated as assets. |
| Source/declarations | `npm run build` | Passed on Node 22 and 24 with the pinned toolchain. |
| Ordinary source and generated parity | `npm run verify` | **453 passed, 0 failed, 0 skipped** on each runtime; regenerated Action files match exactly. |
...
| Packaged consumer | `npm run test:package` | Offline install outside checkout on each runtime; **380-file identical tarball**, CLI, YAML/JSON policies, mock GitHub application, exports and strict types passed. |
...
The ordinary total includes **49 Action-host tests**, **five metadata-contract
checks**, and the CLI repository-base-policy freshness regression added to the
398-test recovered checkpoint. Totals describe executed tests, not a target quota.
The 189 conformance cases are reported separately from the ordinary test count;
they are not 189 additional ordinary tests.

Current transcripts are in `artifacts/verification/actions/`: final runtime,
conformance, shortcut, package and isolated-consumer logs have separately named
boundaries. `consumer.json` records each Action execution, actual runtime, selected
...
the Action-cut evidence. A successful old package test does not qualify modified
package bytes.
...
Both YAML cases now run through the production source loader and original-range
...
See [Action distribution](../../docs/integration/action-distribution.md) for file paths and
[GitHub Actions](../../docs/integration/github-actions.md) for the behavioral contract.
...
unselected. [Publication boundary](../../docs/PUBLICATION-BOUNDARY.md) identifies the current
...
The complete destination remains active in [Direction](../../docs/DIRECTION.md). No public
