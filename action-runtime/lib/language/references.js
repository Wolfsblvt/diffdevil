...
export function boundReferences(node, schema) {
    const result = { metrics: new Set(), bands: new Set(), rules: new Set() };
    const stack = [node];
    while (stack.length) {
            ...
            if (root.kind === 'identifier' && root.binding === 'root') {
                if (root.name === 'metrics' || root.name === 'bands' || root.name === 'rules') {
                    ...
                    if (keys.length)
                        ...
                        if (type && typeof type === 'object' && type.kind === 'record')
        ...
        switch (next.kind) {
            case 'member':
                ...
                break;
            case 'unary':
                stack.push(next.operand);
                break;
            case 'binary':
                stack.push(next.left, next.right);
                break;
            case 'conditional':
                stack.push(next.condition, next.whenTrue, next.whenFalse);
                break;
            case 'call':
                stack.push(...next.arguments);
                break;
            case 'lambda':
            ...
            case 'list':
                ...
                break;
            case 'record':
                stack.push(...next.fields.map(f => f.value));
                break;
        }
    }
