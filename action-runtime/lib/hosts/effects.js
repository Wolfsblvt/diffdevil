...
export function formatEffects(result, format = 'human', verifyOnly = false) {
    ...
    if (!['human', 'markdown', 'agent'].includes(format))
    ...
    for (const observation of result.observations)
        lines.push(`${observation.kind} ${JSON.stringify(observation.subject)}: ${observation.outcome}; readback ${observation.readback}`);
    if ('plan' in result)
        for (const entry of result.plan.held)
            lines.push(`Held rule ${JSON.stringify(entry.rule)}: ${entry.reasons.map(r => r.code).join(', ')}`);
    for (const diagnostic of result.diagnostics)
        lines.push(`${diagnostic.code}: ${JSON.stringify(diagnostic.message)}`);
