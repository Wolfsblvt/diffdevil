...
const horizontal = (character) => character?.value === ' ' || character?.value === '\t';
/** Source provenance only. The YAML library, not this mapper, decides the scalar value. */
export function yamlScalarLocation(node, text, source) {
    const start = node.range?.[0] ?? 0, end = node.range?.[1] ?? start;
    const range = { source, start, end };
    if (typeof node.value !== 'string' || !node.srcToken)
        return { range };
    ...
    if (characters.map(character => character.value).join('') !== node.value)
        return { range };
    return { range, characters, emptyOffset: node.type === 'QUOTE_DOUBLE' || node.type === 'QUOTE_SINGLE' ? start + 1 : end };
}
function rawCharacters(raw, offset) {
    const result = [];
    for (let i = 0; i < raw.length; i++) {
        if (raw[i] === '\r' && raw[i + 1] === '\n') {
            result.push({ value: '\n', start: offset + i, end: offset + i + 2 });
            i++;
        }
        else
            result.push({ value: raw[i], start: offset + i, end: offset + i + 1 });
    }
    return result;
}
function spanCharacter(value, first, last = first) {
...
function flowCharacters(raw, offset, type) {
    ...
    for (let i = 0; i < input.length;) {
        ...
        if (type === 'QUOTE_SINGLE' && current.value === "'" && input[i + 1]?.value === "'") {
            output.push(spanCharacter("'", current, input[i + 1]));
            i += 2;
            continue;
        }
        if (type === 'QUOTE_DOUBLE' && current.value === '\\') {
            ...
            if (['x', 'u', 'U'].includes(next.value)) {
                ...
                for (let unit = 0; unit < value.length; unit++)
                    output.push(spanCharacter(value[unit], current, input[i + width + 1]));
                i += width + 2;
            }
            else {
                output.push(spanCharacter(ESCAPES[next.value] ?? next.value, current, next));
                i += 2;
            }
            continue;
        }
        ...
        let newline = i;
        while (horizontal(input[newline]))
            newline++;
        if (input[newline]?.value === '\n') {
            ...
            while (input[cursor]?.value === '\n') {
                ...
                while (horizontal(input[cursor]))
                    cursor++;
            }
            if (breaks.length === 1)
                output.push(spanCharacter(' ', current, input[cursor - 1]));
            else
                for (let index = 1; index < breaks.length; index++)
                    output.push(spanCharacter('\n', breaks[index]));
            i = cursor;
            continue;
        }
        output.push(current);
...
function linesOf(characters) {
    const lines = [];
    let current = [];
    for (const character of characters) {
        if (character.value === '\n') {
            lines.push({ characters: current, newline: character });
            current = [];
        }
        else
    ...
    if (current.length)
        lines.push({ characters: current });
    return lines;
}
function blockCharacters(raw, text, headerStart, indent, type) {
    ...
    const leading = (line) => line.characters.findIndex(character => character.value !== ' ');
    ...
    for (let i = 0; i < stripped.length; i++) {
        ...
        if (!line.newline)
            continue;
        if (type !== 'BLOCK_FOLDED' || !next)
            output.push(line.newline);
        else if (horizontal(line.characters[0]) || horizontal(next.characters[0]))
            output.push(line.newline);
        else if (line.characters.length && next.characters.length)
            output.push(spanCharacter(' ', line.newline));
...
//# sourceMappingURL=yaml-source-map.js.map