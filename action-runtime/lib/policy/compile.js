...
function validateReferences(document) {
    for (const [id, metric] of Object.entries(document.metrics ?? {})) {
        if (metric.scope !== undefined && !Object.hasOwn(document.scopes ?? {}, metric.scope))
            configError(`Unknown scope ${metric.scope}.`, pointer('/metrics', id), 'E_UNKNOWN_NAME');
        if (metric.scope !== undefined && ['files.total', 'files.excluded'].includes(metric.measure))
            configError('This file count is global-only.', pointer('/metrics', id));
    }
    for (const [id, rule] of Object.entries(document.rules ?? {})) {
        ...
        if (!band)
            configError(`Unknown band ${rule.band}.`, path, 'E_UNKNOWN_NAME');
        const labels = rule.effects?.labels;
        if (!labels)
            continue;
        const group = document.labelGroups?.[labels.group];
        if (!group)
            configError(`Unknown label group ${labels.group}.`, path, 'E_UNKNOWN_NAME');
        const ids = band.ranges.map(range => range.id);
        if (Object.keys(labels.byBand).length !== ids.length || ids.some(id => !Object.hasOwn(labels.byBand, id)))
            configError('byBand must cover exactly the declared band IDs.', path);
        if ([...Object.values(labels.byBand), ...(labels.unknown === undefined ? [] : [labels.unknown])].some(name => !group.includes(name)))
            configError('A mapped label is not a member of its managed group.', path);
    }
}
export function compilePolicy(input = { version: 1 }, options = {}) {
...
export function compilePolicyWithSyntax(input, options, syntax = new Map()) {
    ...
    const result = capture(() => {
        ...
        const metricTypes = Object.fromEntries(Object.keys(document.metrics ?? {}).map(id => [id, 'integer']));
        const parameters = Object.fromEntries(Object.entries(document.parameters ?? {}).map(([id, p]) => [id, p.type]));
        const schema = () => policyEnvironmentSchema({ scopes: Object.keys(document.scopes ?? {}), metrics: metricTypes, bands: Object.keys(document.bands ?? {}), rules: Object.keys(document.rules ?? {}), parameters });
        const parse = (text, path) => located(path, () => {
        ...
        const bind = (node, path, context, environment = schema()) => located(path, () => unwrap(compileAst(node, { environment, context, limits })));
        ...
        for (const id of order) {
            ...
            if (!isNumeric(program.resultType))
                configError('A metric must produce a numeric measurement.', path, 'E_TYPE');
            metricTypes[id] = program.resultType;
        }
        const finalSchema = schema();
        const metrics = new Map(order.map(id => { const { node, path } = metricSyntax.get(id); return [id, bind(node, path, 'metric', finalSchema)]; }));
        const bands = new Map();
        ...
        const conditions = new Map(), queries = new Map();
        ...
        const files = inertCopy(options.templateFiles ?? {}, { code: 'E_CONFIG' });
        const templates = new Map(), resolvedTemplates = {};
        for (const id of Object.keys(document.rules ?? {}).sort()) {
            ...
            if (typeof text !== 'string')
                configError(`The host must supply trusted text for template file ${comment.templateFile}.`, path, 'E_TEMPLATE_SOURCE');
        ...
        const program = deepFreeze({ kind: 'diffdevil.policy', id: contentId('policy', { document, templates: resolvedTemplates, semantics: normalized.semantics }), semantics: normalized.semantics, requiredParameters: Object.keys(document.parameters ?? {}).filter(id => document.parameters[id].required).sort() });
