...
import { actionContext, choice, positiveInteger } from './inputs.js';
import { loadActionPolicy } from './policy.js';
import { ActionOutputSession, actionOutputs, outputCommands } from './outputs.js';
function selectedDisplay(policy, report, context) {
    ...
    const selector = !outputSelection ? expression : context.inputs.formula ?? { kind: 'query', metric: context.inputs.metric ?? 'changed',
    ...
    if (value.kind !== 'number')
        fail('E_INTERNAL', 'Selected Action metric must be numeric.', 'evaluate');
...
export function runAction(entryPoint, options = {}) {
    return captureAsync(async () => {
        ...
        try {
            ...
            if (entryPoint === 'sync-labels') {
                ...
                const target = { repository: context.repository, pullRequest: context.pullRequest };
                ...
                if (suppliedReport)
                    assertReportMatchesPolicy(policy, suppliedReport, evaluated, { parameters: context.parameters });
