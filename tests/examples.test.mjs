...
const patch = await read('examples/diffs/review.diff');
const report = unwrap(analyzeDiff(patch));
const policy = async name => unwrap(compilePolicy(unwrap(readPolicyYaml(await read(`examples/policies/${name}.yml`)))));
...
async function workspace(t) {
  ...
  await cp(join(root, 'examples'), join(dir, 'examples'), { recursive: true });
  return dir;
}
...
async function actionFixture(t, name) {
  const dir = await workspace(t);
  const workflow = parse(await read(`examples/workflows/${name}.yml`));
  const step = Object.values(workflow.jobs)[0].steps[0];
  const entry = step.uses.includes('/analyze@') ? 'analyze' : 'root';
  const metadata = parse(await read(entry === 'root' ? 'action.yml' : 'analyze/action.yml'));
  ...
  if (step.with?.config) fake.contents.set(`${BASE}:${step.with.config}`, await read(`examples/policies/${name}.yml`));
...
test('all maintained workflow step inputs are declared, and applying examples avoid checkout/run', async () => {
  ...
  const files = (await readdir(join(root, 'examples/workflows'))).filter(name => name.endsWith('.yml'));
  assert.ok(files.length > 0);
  for (const file of files) {
    const workflow = parse(await read(`examples/workflows/${file}`));
    for (const job of Object.values(workflow.jobs)) for (const step of job.steps) {
      ...
      const entry = step.uses.split('@')[0].split('/')[2];
