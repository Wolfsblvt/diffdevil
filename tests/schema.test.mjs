...
const report = JSON.parse(readFileSync('examples/reports/exact.json', 'utf8'));
...
test('the complete compiler works when Node forbids runtime string code generation', () => {
  const source = `import {readPolicyYaml,compilePolicy,evaluatePolicyQuery,unwrap} from './dist/lib/index.js';
    ...
    const result=unwrap(evaluatePolicyQuery(policy,JSON.parse(readFileSync('examples/reports/exact.json','utf8')),{query:'arithmetic'}));
