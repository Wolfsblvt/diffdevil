...
for (const row of JSON.parse(readFileSync('spec/detail/v1/conformance/source-positions.json', 'utf8')).cases.filter(row => row.container)) {
...
test('YAML, JSON-compatible input and objects converge on the same compiled policy', () => {
  ...
  for (const filename of ['minimal', 'thresholds', 'full', 'comments-opt-in']) {
    const data = unwrap(readPolicyYaml(readFileSync(`examples/policies/${filename}.yml`, 'utf8')));
