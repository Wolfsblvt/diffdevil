...
test('require-resolved emits a held plan with exit 3 and preserves unknown evidence', async t => {
  ...
  const args = ['plan', '--report', join(process.cwd(), 'examples/reports/bounded.json'), '--config', 'policy.yml', '--target-repo', TARGET.repository, '--target-pr', '42', '--require-resolved', '--format', 'json'];
