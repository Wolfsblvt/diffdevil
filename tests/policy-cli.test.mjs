...
const root=resolve('.'),bin=resolve('dist/lib/cli/main.js'),report=resolve('examples/reports/exact.json');
...
test('plan failure and unresolved configured values cannot overwrite an existing output file',()=>home(dir=>{
 const bounded=resolve(root,'examples/reports/bounded.json');
