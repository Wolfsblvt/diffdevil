...
for(const row of JSON.parse(readFileSync(new URL('../spec/detail/v1/conformance/machine-output.json',import.meta.url))).cases){
...
test('canonical reports and environment output preserve bounded evidence',()=>{
 const report=unwrap(readReport(JSON.parse(readFileSync(new URL('../examples/reports/bounded.json',import.meta.url)))));
