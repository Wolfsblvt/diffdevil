...
const script = fileURLToPath(new URL('../scripts/test.mjs', import.meta.url));
function runFixture(files) {
  ...
  try {
    ...
    for (const [name, body] of Object.entries(files)) writeFileSync(join(directory, 'tests', name), body);
...
test('ordinary runner rejects an empty test file even beside passing tests', () => {
  ...
  assert.match(result.stderr, /did not report registered test cases: tests[/\\]empty\.test\.mjs/);
});
