...
try {
  ...
  await cp(join(root, 'action-runtime'), join(distribution, 'action-runtime'), { recursive: true });
  const metadata = {};
  for (const entry of entries) {
    const source = entry === 'root' ? root : join(root, entry), destination = entry === 'root' ? distribution : join(distribution, entry);
  ...
  await cp(join(root, 'tests/helpers/github.mjs'), join(home, 'github-fixture.mjs'));
...
`);
  ...
  async function action(node, entry, inputs = {}, { scenario = 'ordinary', expectedExit = 0, expectOutputs = true } = {}) {
    ...
    const entryRoot = entry === 'root' ? distribution : join(distribution, entry);
  ...
  const manifest = JSON.parse(await readFile(join(distribution, 'action-runtime/MANIFEST.json'), 'utf8'));
