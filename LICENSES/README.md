...
The product name, slogans, logos, mascot, artwork, screenshots, and other brand or visual assets remain reserved, including the identity SVGs under `design/assets/identity/` and the rasters derived from them under `apps/website/public/`. IBM Plex Sans and Mono are redistributed by the website build under the SIL Open Font License. Their appearance in CC-licensed prose does not grant trademark or asset rights. `docs/BRANDING.md` describes the textual brand; it is not a licence for the brand identifiers it describes. The local playground application is present in source; no hosted deployment or GitHub App runtime is claimed yet.
