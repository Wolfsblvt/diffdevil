...
`migrations/0001_initial.sql` and `0002_execution_lifecycle.sql` establish installation/repository state, atomic delivery and renewable fenced per-PR leases, owned-check handles, seven-day operational records, opted-in numeric history, repair work, and restore-resistant deletion tombstones. Before a real adapter is operated, run D1 migrations against a deliberately selected binding and use a disposable fixture to exercise duplicate delivery, lease recovery, DLQ, history repair, and export/import. The exact `database_id` in `wrangler.jsonc` is a non-operational placeholder: an operator must replace it only after creating or selecting the target D1 database and read back the binding.

Operational results are minimized and intended for seven-day recovery. A repository remains `pending-enable` after lifecycle discovery: provider execution and history both need positive current access and an explicit operator configuration change. Opted-in history stores an allowlisted versioned aggregate plus pathless per-file numeric rows and effect request/readback standing only; paths, previous paths, people, prose, raw reports, source, queues, errors, templates, and secrets are excluded. A failed history publication becomes separate repair work and never replays already-observed provider effects. History retention/offboarding, independent administrator grace, export, deletion, and tombstone reapplication remain governed by [the privacy contract](../../docs/PRIVACY-AND-DATA.md).
