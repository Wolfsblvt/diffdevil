...
async function fixture() {
  ...
  const providerState = { userValid: true, installation: 'active', repository: 'available', canAdminister: true, checks: 0 };
  const provider = {
    ...
    exchangeCode: async ({ code }) => { assert.equal(code, CODE); return { material: { accessToken: TOKEN, refreshToken: 'refresh-private', body: PROVIDER_BODY }, expiresAt: '2026-09-23T01:00:00.000Z' }; },
    ...
    verifyUserAuthorization: async ({ userId, material }) => {
      ...
      return providerState.refreshOnVerify
  ...
  const service = createAuthorizationService({ store, admission, provider, protector, returnContexts: [CONTEXT, 'account-home'], allowedOrigins: [ORIGIN], allowedCallbackUrls: [CALLBACK], now: () => clock.value });
  return { runtime, database, appStore, store, service, clock, providerState };
}

async function signIn(service) {
  const { state } = await service.begin(CONTEXT);
  const { artifact } = await service.complete({ state, code: CODE, returnContext: CONTEXT });
  ...
  assert.ok(session);
  return { state, artifact, session, result };
}

test('state and return context refuse open redirects, mismatch, expiry and replay', async () => {
  const source = await fixture();
  try {
    const { service, clock } = source;
    ...
    await assert.rejects(service.complete({ state: first.state, code: CODE, returnContext: 'account-home' }), { code: 'E_AUTH_STATE' });
    await assert.rejects(service.complete({ state: first.state, code: CODE, returnContext: 'other' }), { code: 'E_AUTH_RETURN_CONTEXT' });
    const valid = await service.complete({ state: first.state, code: CODE, returnContext: CONTEXT });
    await assert.rejects(service.complete({ state: first.state, code: CODE, returnContext: CONTEXT }), { code: 'E_AUTH_STATE' });
    ...
    const short = await service.complete({ state: artifactExpires.state, code: CODE, returnContext: CONTEXT });
    ...
    await assert.rejects(service.complete({ state: expiring.state, code: CODE, returnContext: CONTEXT }), { code: 'E_AUTH_STATE' });
...
test('callback, one-time artifact and rotated session preserve consent and do not persist browser secrets', async () => {
  ...
  try {
    ...
    const { state, artifact, session, result } = await signIn(service);
    ...
    for (const privateValue of [state, artifact, session, CODE, TOKEN, PROVIDER_BODY, REPOSITORY_NAME]) assert.equal(persisted.includes(privateValue), false);
...
test('mutations require a same-origin non-GET request and current administrator authority', async () => {
  ...
  try {
    ...
    const settings = { origin: 'dashboard', revision: initial.body.revision, writer: { confidence: 'administrator-declared' } };
    ...
    assert.equal(providerState.checks, 3, 'current access is reacquired on every authorized admission attempt');
