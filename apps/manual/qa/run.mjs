...
import { editRef } from '../source-resolver.mjs';
...
try {
 await check('Every selected manual route has its actual H1 and exact source/edit provenance',async()=>{
  for (const selected of manualPages) {
   ...
   await expect(page.locator('.docs-provenance a').filter({hasText:'Edit on GitHub'})).toHaveAttribute('href',`https://github.com/Wolfsblvt/diffdevil/edit/${editBranch}/${selected.source}`);
