...
import { json, planJson, targetFor, type Evaluation } from './engine';
import { exportCli } from './export-cli';
...
export function ExportDialog({ evaluation, acquired, policyText, onClose }: Props) {
  ...
  const [artifact, setArtifact] = useState<'report' | 'plan'>('report');
  ...
  const text = tab === 'cli' ? cli : tab === 'step' ? step : tab === 'workflow' ? workflow : tab === 'policy' ? policyText : artifact === 'plan' && evaluation.plan ? planJson(evaluation.plan) : json(evaluation.report);
  ...
  return (
    <dialog ref={dialog} className="sheet" aria-labelledby="export-h">
      ...
      <div className="sheet-body">
        ...
        {tab === 'cli' && <div className="panel"><strong>Reads only.</strong> Save report.json from the data tab first. These commands replay that report, not a new local comparison or current PR head. {isPreset ? 'Planning uses size@1 explicitly.' : 'Save the current .diffdevil.yml from its tab before planning.'}</div>}
        {tab === 'data' && <label className="field-label">Artifact <select className="select" value={artifact} onChange={event => setArtifact(event.target.value as 'report' | 'plan')}><option value="report">report.json</option>{evaluation.plan && <option value="plan">plan.json</option>}</select></label>}
        ...
        <div className="panel">
          ...
          <ul className="small pg-assumptions">
            <li>{sourceNote}. The report export preserves this acquired comparison; it is not a freshness or write-authority receipt.</li>
