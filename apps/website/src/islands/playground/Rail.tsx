...
/** The rail: input tabs, public-PR input or example cards, then configuration. Desktop rail never collapses; ≤ 900 it becomes a sticky bottom sheet. */
...
export function Rail(props: Props) {
  ...
  const selectKind = (next: 'fixtures' | 'prs') => {
    setKind(next);
    const first = (next === 'fixtures' ? fixtures : curated)[0];
    if (first && !(next === 'fixtures' ? fixtures : curated).some(e => e.id === state.example)) props.onSelectExample(first.id);
  };

  const config = (
    <section className="pg-config" aria-labelledby="pg-config-h">
      <div className="pg-config-head">
        <h2 id="pg-config-h" className="lane">{c.config}</h2>
      ...
      <p className="caption">{c.configNote}</p>
    </section>
  );
    ...
    <>
      ...
      <div id="panel-input" role="tabpanel" aria-labelledby={state.mode === 'pr' ? 'tab-pr' : 'tab-examples'} className="pg-input">
        {state.mode === 'pr' ? (
          ...
          <div className="stack">
            <div className="chips" role="radiogroup" aria-label="Example kind">
              <button type="button" role="radio" className="chip" aria-checked={kind === 'fixtures'} onClick={() => selectKind('fixtures')}>{c.fixtures} · {fixtures.length}</button>
              ...
              {(kind === 'fixtures' ? fixtures : curated).map(card => (
                <li key={card.id}>
                  <button type="button" className="pg-card" aria-pressed={state.example === card.id} onClick={() => props.onSelectExample(card.id)}>
                    {card.group === 'fixture'
                      ? <><span className="pg-card-glyph" aria-hidden="true">{card.glyph}</span><span className="pg-card-body"><span className="pg-card-title">{card.title}</span><span className="pg-card-hook">{card.hook}</span>{card.meta && <span className="pg-card-meta">{card.meta}</span>}</span></>
                      : <><span className="pg-card-glyph" aria-hidden="true"><GitHubMark /></span><span className="pg-card-body"><span className="pg-card-title">{card.repository} · PR #{card.pullRequest}</span><span className="pg-card-hook">{card.reason}</span><span className="pg-card-meta">snapshot {card.head?.slice(0, 7)} · {card.analyzedAt?.slice(0, 10)} <span className="chip-ev" data-status={card.evidence}>{card.evidence}</span></span></span></>}
                  </button>
                </li>
              ))}
              {kind === 'prs' && curated.length === 0 && <li className="caption">No curated pull requests yet.</li>}
            </ul>
          </div>
        )}
      </div>
