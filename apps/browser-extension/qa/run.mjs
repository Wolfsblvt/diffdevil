...
try {
  ...
  await check('Report popover is one persistent, non-modal anchored surface', async () => { await aggregate.click(); const panel = github.locator('.ddx-popover'); await panel.waitFor(); assert.equal(await panel.getAttribute('role'), 'dialog'); assert.equal(await panel.getAttribute('aria-modal'), null); assert.ok((await panel.innerText()).includes('Changed')); assert.equal(await github.evaluate(() => document.body.style.overflow), ''); assert.equal(await github.locator('.ddx-popover').count(), 1); });
  ...
  await check('File acquisition failure is bounded instead of an infinite retry loop', async () => { failFiles = true; const before = fileCalls; await stale.page.evaluate(() => { void controller.refresh(); }); await stale.page.waitForTimeout(150); assert.equal(fileCalls - before, 1); await stale.page.waitForTimeout(150); assert.equal(fileCalls - before, 1); assert.ok((await stale.page.locator('.ddx-status').textContent()).includes('Fixture file transport')); failFiles = false; });
