...
A result already computed for the same repository, pull request, base and head is reattached at once when you move between Conversation, Commits, Checks and Files changed or reload; freshness is confirmed in the background, and only a changed head triggers `⟳ re-reading`. Private pull requests are read through the signed-in `/changes` page's own file summaries and embedded diff contents, without a token or any new permission; a file whose content GitHub did not embed is shown with bounded evidence rather than a guess.
