...
import { sourceResolverUrl } from '../apps/manual/source-resolver.mjs';
...
const selected = retained.filter(entry => !entry.repositoryOnly && existsSync(join(root, entry.source)));
...
function rewriteTarget(target, source) {
  ...
  if (entry?.repositoryOnly) return { href: sourceResolverUrl(resolvedPath) + (hash ? `#${hash}` : ''), asset: resolvedPath };
...
export const sidebar = groups.map(group => ({ label: group.label, items: group.entries.flatMap(entry => {
  ...
  if (entry.repositoryOnly) return [{ label: titleOf(readFileSync(join(root, entry.source), 'utf8'), entry), link: sourceResolverUrl(entry.source) }];
