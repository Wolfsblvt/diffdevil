...
`@v1` follows the maintained Action major. Select a verified full commit SHA when your repository requires immutable source. The Action includes its runtime; the job does not run `npm install`.
