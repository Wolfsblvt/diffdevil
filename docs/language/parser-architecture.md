...
## Coordinate and hostile-input qualification

At the parser boundary, adapt toolkit token/CST endpoints into exclusive UTF-16 ranges. The selected Chevrotain major uses `-1` for unavailable positions; old NaN-only checks are insufficient. Omit unavailable coordinates rather than fabricating zero. Keep the dated [implementation guide](../reference/2026-09-09/chevrotain-implementation-guide.md) and [toolkit evidence](../reference/2026-09-09/sources-and-research.md#c1) as implementation context, not a new language version. The [numeric reference evidence](../reference/2026-09-09/sources-and-research.md#n1) supplies the binary64/safe-integer background without changing the product's explicit numeric profile.

Production lexing and parsing reject every error; recovered editor trees have no execution route. Bound nesting during parsing, not after a stack overflow. Token patterns must not admit catastrophic backtracking: string scanning can be linear, with explicit malformed-escape and numeric-boundary checks. Globs use the finite pattern grammar and bounded state exploration or dynamic programming. Charge visited matching states; never translate caller patterns into an unbounded backtracking regular expression.

The evaluator accepts normalized inert data, not arbitrary JavaScript instances. It must not invoke getters, traverse prototypes, call host methods, or coerce an object through `toString`. Safe own-data maps, forbidden-key checks and immutable records supplement that boundary rather than replacing it. A TypeScript caller already executing arbitrary code in the process is outside an expression sandbox's protection.

There is no expression route to shell, filesystem, network, environment, time, randomness, dynamic imports, reflection, `eval`, or user callbacks. Fixed product JSON Schemas may be compiled into standalone validators at build time; user-supplied schemas are not compiled during evaluation. See the dated [validator evidence](../reference/2026-09-09/sources-and-research.md#j1).

Qualification covers malformed and deep syntax, unsafe numbers and strings, prototype-oriented names, extreme intervals, nested collections, hostile paths, YAML aliases and source maps, strict-output transport, and privileged policy loading. Conformance fixtures do not certify an unbuilt runtime. Deterministic work charging, including cached metric references and multiplicative nested work, is separate from wall-clock measurements; [cost-budget research](../reference/2026-09-09/sources-and-research.md#k1) does not turn the selected default budgets into measured capacity claims. Current reader contracts are [detail](../manual/reference/language-and-contracts/detail-language.md) and [Troubleshooting](../manual/help/troubleshooting.md).
