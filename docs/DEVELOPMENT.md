...
These are selected reproducible versions, not a claim that every package is
latest or independently security-audited.
...
| `npm run playground:worker` | Builds the reusable engine and starts the same public route through local Wrangler/workerd. It does not contact Cloudflare. |
| `npm run check:workers` | Bundles `wrangler.jsonc` and the static asset binding with `wrangler deploy --dry-run`. It creates no Worker, version, preview, deployment, route or domain. |
