...
import { analyzeGit, type GitOptions } from '../sources/git.js';
import { analyzeChanges, type AnalyzeOptions } from '../report.js';
import type { DiffSourceIdentity, PlanTarget, Report, Result } from '../model.js';
import { parsePatch, parseUnifiedDiff, type ChangeInput } from '../sources/patch.js';
...
export interface PullSnapshot { readonly head: string; readonly base: string; readonly changedFiles: number; readonly state: 'open' | 'closed' }
/** Read the current target, not an event's potentially stale head or base. */
export async function readPullSnapshot(client: GitHubClient, target: PlanTarget): Promise<PullSnapshot> {
  const pr = githubRecord(await client.json(pullPath(target)), 'pull request');
  if (pr.number !== target.pullRequest) fail('E_PLAN_TARGET', 'GitHub returned a different pull request.', 'source');
  const base = githubRecord(pr.base, 'base'), head = githubRecord(pr.head, 'head');
  const repo = githubRecord(base.repo, 'base repository');
  if (githubString(repo.full_name, 'base repository name').toLowerCase() !== target.repository.toLowerCase()) fail('E_PLAN_TARGET', 'GitHub returned a different base repository.', 'source');
  if (pr.state !== 'open' && pr.state !== 'closed') fail('E_GITHUB_RESPONSE', 'Unknown pull-request state.', 'source');
...
export function githubSourceIdentity(client: GitHubClient, target: PlanTarget, snapshot: PullSnapshot): DiffSourceIdentity {
...
function changeFromApi(input: unknown): ChangeInput {
...
async function refineKinds(client: GitHubClient, target: PlanTarget, changes: readonly ChangeInput[]): Promise<readonly ChangeInput[]> {
  if (!changes.some(change => change.kind === 'unknown')) return changes;
  let diff: string;
  try { diff = (await client.request(pullPath(target), { accept: 'application/vnd.github.diff' })).text; }
  catch (error) {
    // This optional read improves evidence only; a denied/missing raw diff does not erase the API facts.
    if (error instanceof GitHubRequestError) return changes;
    throw error;
  }
  const proofs = new Map<string, ChangeInput>(), duplicates = new Set<string>();
  try {
    for (const proof of parseUnifiedDiff(diff)) proofs.set(proof.path, proof);
  } catch { /* Fall back to independently complete file blocks, not an alternate diff parser. */ }
  if (proofs.size === 0) for (const fragment of diff.split(/(?=^diff --git )/mu).filter(Boolean)) {
    try {
      const parsed = parseUnifiedDiff(fragment);
      if (parsed.length === 1) {
        const proof = parsed[0]!;
        if (proofs.has(proof.path)) duplicates.add(proof.path);
  ...
  return changes.map(change => {
    if (change.kind !== 'unknown') return change;
    const proof = proofs.get(change.path);
    if (!proof || duplicates.has(change.path) || proof.oldPath !== change.oldPath || proof.changeType !== change.changeType) return change;
    if (proof.kind === 'binary' || proof.kind === 'submodule') return { ...change, kind: proof.kind };
    if (proof.patch && proof.additions === change.additions && proof.deletions === change.deletions) return { ...change, kind: 'text', patch: proof.patch };
    return change;
  });
}
/** Paginated PR acquisition with explicit completeness and before/after identity checks. */
export function analyzeGitHub(client: GitHubClient, target: PlanTarget, options: Omit<AnalyzeOptions, 'source' | 'fileSet'> = {}): Promise<Result<Report>> {
  return captureAsync(async () => {
    ...
    if (before.head !== after.head || before.base !== after.base || before.changedFiles !== after.changedFiles) fail('E_SOURCE_STALE', 'Pull-request head or base changed during acquisition. Analyze the current comparison.', 'source');
    if (changes.length > before.changedFiles) fail('E_GITHUB_RESPONSE', 'GitHub returned more files than the pull-request total.', 'source');
    if (pages.truncated && changes.length === before.changedFiles) fail('E_GITHUB_RESPONSE', 'GitHub pagination contradicts the complete file count.', 'source');
...
export function analyzeGitHubGit(client: GitHubClient, target: PlanTarget, options: Pick<GitOptions, 'cwd' | 'paths' | 'scopes' | 'maximumBytes'> = {}): Promise<Result<Report>> {
  return captureAsync(async () => {
    const before = await readPullSnapshot(client, target);
    const measured = unwrap(await analyzeGit({ ...options, base: before.base, head: before.head, comparison: 'three-dot' }));
    const after = await readPullSnapshot(client, target);
    if (before.head !== after.head || before.base !== after.base) fail('E_SOURCE_STALE', 'The PR head or base moved during local Git acquisition. Analyze the current comparison again.', 'source');
