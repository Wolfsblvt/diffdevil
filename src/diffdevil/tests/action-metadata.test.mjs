...
const catalog = JSON.parse(await readFile(resolve(root, 'src/diffdevil/contracts/detail/v1/action-surface.json'), 'utf8'));
for (const entry of ENTRY_POINTS) test(`${entry} metadata declares precisely the supported inputs/outputs and a shipped Node 24 entry`, async () => {
  const path = resolve(root, entry === 'root' ? 'action.yml' : `actions/${entry}/action.yml`);
