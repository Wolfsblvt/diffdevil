...
const cases = suite => load(`src/diffdevil/contracts/detail/v1/conformance/${suite}.json`).cases;
const empty = unwrap(createEnvironment({}, {}));
function environment(row) {
  if (!row.environment || row.environment === 'empty') return empty;
  if (row.environment.startsWith('report-')) return unwrap(environmentFromReport(load(`docs/examples/reports/${row.environment.slice(7)}.json`)));
