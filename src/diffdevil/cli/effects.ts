...
import { applyGitHubPolicy, type GitHubApplyOptions } from '../github/apply.js';
...
import { loadHostedGitHubPolicy, type HostedPolicySelection } from '../hosts/github-policy.js';
...
export async function runEffectCommand(invocation: Invocation, cwd: string, client: GitHubClient): Promise<Rendered> {
  ...
  if (inputPlan && (repository !== inputPlan.target.repository || pullRequest !== inputPlan.target.pullRequest)) fail('E_PLAN_TARGET', 'Explicit target contradicts the saved plan.', 'plan');
  ...
  const selection: HostedPolicySelection = {
    ...
    ...(v['policy-source'] === undefined ? {} : { source: String(v['policy-source']) as HostedPolicySelection['source'] & string }),
  ...
  if (invocation.command.startsWith('labels ')) {
    ...
    const mode = verify ? 'verify' : String(v.definitions ?? 'sync') as 'ensure' | 'sync';
  ...
  if (pullRequest === undefined) fail('E_PLAN_TARGET', 'Application requires a pull-request target.', 'plan');
  const parameters = await readParameters(hosted.policy, v, cwd);
  const options: GitHubApplyOptions = {
    parameters,
    definitions: String(v.definitions ?? planDefinitionMode(inputPlan) ?? 'ensure') as NonNullable<GitHubApplyOptions['definitions']>,
