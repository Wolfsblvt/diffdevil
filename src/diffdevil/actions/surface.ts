...
export const ENTRY_POINTS = ['root', 'analyze', 'apply', 'sync-labels'] as const;
export type ActionEntryPoint = typeof ENTRY_POINTS[number];
interface InputDefinition { readonly description: string; readonly entries: readonly ActionEntryPoint[]; readonly default?: string }
const all = ENTRY_POINTS, pr = ['root', 'analyze', 'apply'] as const, effects = ['root', 'apply'] as const;
...
export const NUMERIC_OUTPUTS = ['lines-added', 'lines-deleted', 'lines-modified', 'lines-changed', 'raw-added', 'raw-deleted', 'raw-churn', 'files-total', 'files-included', 'files-excluded', 'files-unmeasurable'] as const;
...
export const PLAN_OUTPUTS = ['plan-json', 'plan-path'] as const;
export const EFFECT_OUTPUTS = ['effects-changed', 'effects-status', 'effects-path'] as const;
export function inputNames(entry: ActionEntryPoint): string[] {
  return Object.keys(ACTION_INPUTS).filter(name => ACTION_INPUTS[name]!.entries.includes(entry));
}
export function outputNames(entry: ActionEntryPoint): readonly string[] {
  return entry === 'sync-labels' ? EFFECT_OUTPUTS : [...REPORT_OUTPUTS, ...(entry === 'analyze' ? [] : [...PLAN_OUTPUTS, ...EFFECT_OUTPUTS])];
}
export const SHORTCUT_INPUTS = ['metric', 'threshold', 'comparison', 'files', 'scope', 'path', 'exclude', 'include-only', 'force-include', 'exclude-mode', 'include-only-mode', 'force-include-mode', 'label', 'remove-label-when-false', 'comment-template', 'comment-mode', 'formula', 'condition'] as const;
