...
import { open, lstat, realpath, mkdir, mkdtemp, access, unlink, type FileHandle } from 'node:fs/promises';
...
import type { BandResult, Decision, EffectPlan, NumericMeasurement, Report } from '../model.js';
import type { GitHubApplyResult } from '../github/apply.js';
import type { DefinitionResult } from '../github/labels.js';
import type { ActionContext } from './inputs.js';
import { outputNames, type ActionEntryPoint } from './surface.js';
export type ActionEffects = GitHubApplyResult | DefinitionResult;
export interface ActionResult {
  readonly report?: Report; readonly plan?: EffectPlan; readonly effects?: ActionEffects;
  readonly metric?: NumericMeasurement; readonly decision?: Decision; readonly band?: BandResult;
  readonly exitCode: 0 | 1 | 2;
}
/** Escape command data, including newlines, without ever writing policy or patches as commands. */
export function escapeCommand(value: string): string {
  return value.replaceAll('%', '%25').replaceAll('\r', '%0D').replaceAll('\n', '%0A');
}
/** Each value gets an unguessable delimiter, checked against every represented line. */
export function outputCommands(outputs: Readonly<Record<string, string>>): string {
  let text = '';
  for (const [name, value] of Object.entries(outputs)) {
    if (!/^[a-z][a-z0-9-]*$/u.test(name) || value.includes('\0')) fail('E_ACTION_OUTPUT', 'Invalid Action output name or NUL-containing value.', 'format');
    let delimiter: string;
    do { delimiter = `diffdevil_${randomUUID()}`; } while (value.split(/\r?\n/u).includes(delimiter));
    text += `${name}<<${delimiter}\n${value}\n${delimiter}\n`;
  }
  // Leave room for other steps: GitHub approximates its per-job budget in UTF-16.
  if (text.length * 2 > 512 * 1024) fail('E_LIMIT', 'Action outputs exceed the selected 512 KiB UTF-16 budget. Full results remain in artifact files.', 'format');
  return text;
}
function numeric(outputs: Record<string, string>, base: string, value?: NumericMeasurement, valueName = base): void {
...
function compactEvidence(value: NumericMeasurement | Decision | BandResult | undefined): object | undefined {
  if (value === undefined) return undefined;
  if (value.status !== 'unknown' && value.status !== 'unmeasurable') return value;
...
function compactMeasurements(values: Readonly<Record<string, NumericMeasurement>>): object {
  return Object.fromEntries(Object.entries(values).map(([name, value]) => [name, compactEvidence(value)]));
}
export interface ArtifactPaths { readonly report?: string; readonly plan?: string; readonly effects?: string }
/** Compact discriminated envelopes are not truncated full reports or executable plans. */
export function actionOutputs(entry: ActionEntryPoint, result: ActionResult, paths: ArtifactPaths): Record<string, string> {
  const outputs: Record<string, string> = Object.fromEntries(outputNames(entry).map(name => [name, '']));
  if (result.report) {
    const report = result.report;
    for (const key of ['added', 'deleted', 'modified', 'changed'] as const) numeric(outputs, `lines-${key}`, report.totals.lines[key]);
    for (const key of ['added', 'deleted', 'churn'] as const) numeric(outputs, `raw-${key}`, report.totals.raw[key]);
    for (const key of ['total', 'included', 'excluded', 'unmeasurable'] as const) numeric(outputs, `files-${key}`, report.totals.files[key]);
...
export function actionSummary(result: ActionResult): string {
  const rows: [string, string][] = [];
  if (result.report) {
    const { source, totals } = result.report;
    rows.push(['Source', source.kind]);
    if (source.repository) rows.push(['Repository', source.repository]);
    if (source.pullRequest) rows.push(['Pull request', String(source.pullRequest)]);
    if (source.comparison) rows.push(['Comparison', source.comparison]);
    if (source.base) rows.push(['Compared base', source.base]);
    if (source.baseTip) rows.push(['Base tip', source.baseTip]);
    if (source.head) rows.push(['Head', source.head]);
    rows.push(['Measurement', result.report.measurement.status],
      ['Changed lines', displayMeasurement(totals.lines.changed)],
      ['Added-only lines', displayMeasurement(totals.lines.added)],
      ['Deleted-only lines', displayMeasurement(totals.lines.deleted)],
      ['Modified lines', displayMeasurement(totals.lines.modified)],
      ['Raw additions', displayMeasurement(totals.raw.added)],
      ['Raw deletions', displayMeasurement(totals.raw.deleted)],
      ['Raw churn', displayMeasurement(totals.raw.churn)]);
    for (const name of ['total', 'included', 'excluded', 'unmeasurable'] as const) {
    ...
    if (result.metric) rows.push(['Metric', displayMeasurement(result.metric)]);
    if (result.decision) rows.push(['Decision', result.decision.status === 'resolved' ? String(result.decision.value) : 'unknown']);
    if (result.band) rows.push(['Band', result.band.status === 'resolved' ? result.band.id : 'unknown']);
  }
  if (result.plan) rows.push(['Desired operations (not observed changes)', String(result.plan.operations.length)], ['Held rules', String(result.plan.held.length)]);
  if (result.effects) rows.push(['Provider result', result.effects.status], ['Verified changed operations', String(result.effects.changed)]);
  else rows.push(['Provider effects', 'Not applied']);
  ...
  if (result.report?.metrics && Object.keys(result.report.metrics).length) {
    ...
    text += '\n### Policy metrics\n\n| Metric | Value |\n| --- | --- |\n';
    for (const [name, value] of metrics.slice(0, 30)) text += `| ${html(name)} | ${html(displayMeasurement(value))} |\n`;
    if (metrics.length > 30) text += '\nAdditional metrics are in the full report.\n';
  }
  ...
  if (result.effects) {
    ...
    for (const observation of observations.slice(0, 40)) text += `- ${html(observation.kind)}: ${html(observation.subject)} — ${html(observation.outcome)}; request ${html(observation.request)}, readback ${html(observation.readback)}.\n`;
...
async function commandFile(path: string | undefined, name: string): Promise<FileHandle> {
  if (!path) fail('E_ACTION_ENV', `${name} must identify a runner command file.`, 'config');
  try {
    if (!(await lstat(path)).isFile()) fail('E_ACTION_ENV', `${name} must be an existing regular file, not a symlink.`, 'config');
    return await open(path, 'a');
  } catch { return fail('E_ACTION_ENV', `${name} must identify an existing writable regular file.`, 'config'); }
}
...
export class ActionOutputSession {
  private constructor(readonly paths: ArtifactPaths, private readonly output: FileHandle, private readonly summary: FileHandle | undefined) {}
  static async open(context: ActionContext, environment: NodeJS.ProcessEnv): Promise<ActionOutputSession> {
    const output = await commandFile(environment.GITHUB_OUTPUT, 'GITHUB_OUTPUT');
    let summary: FileHandle | undefined;
    try {
      if (context.summary) {
        ...
        if (outStat.dev === summaryStat.dev && outStat.ino === summaryStat.ino) fail('E_ACTION_PATH', 'GITHUB_OUTPUT and GITHUB_STEP_SUMMARY must be distinct files.', 'config');
      }
      const root = environment.RUNNER_TEMP ?? tmpdir();
      await mkdir(root, { recursive: true });
      const directory = await mkdtemp(resolve(root, 'diffdevil-'));
      const paths: { report?: string; plan?: string; effects?: string } = {};
      const sources = [environment.GITHUB_OUTPUT, context.summary ? environment.GITHUB_STEP_SUMMARY : undefined, environment.GITHUB_EVENT_PATH,
        ...
        context.inputs['policy-source'] === 'workspace' && context.inputs.config ? resolve(context.cwd, context.inputs.config) : undefined].filter((path): path is string => Boolean(path));
      const occupied = new Set(await Promise.all(sources.map(canonicalDestination)));
      for (const kind of ['report', 'plan', 'effects'] as const) {
        ...
        if (occupied.has(canonical)) fail('E_ACTION_PATH', 'Artifact destinations must be distinct from one another and from input/runner files.', 'config');
        occupied.add(canonical);
        try { if (!(await lstat(path)).isFile()) fail('E_ACTION_PATH', 'An artifact destination must be a regular file, not a directory or symlink.', 'config'); }
    ...
    } catch (error) { await output.close(); await summary?.close(); throw error; }
  }
  async publish(entry: ActionEntryPoint, result: ActionResult): Promise<Readonly<Record<string, string>>> {
    // Journal first: a later filesystem failure must not discard already observed remote effects.
    if (this.paths.effects && result.effects) await writeAtomic(this.paths.effects, JSON.stringify(result.effects, null, 2) + '\n');
    if (this.paths.report && result.report) await writeAtomic(this.paths.report, JSON.stringify(result.report, null, 2) + '\n');
    if (this.paths.plan && result.plan) await writeAtomic(this.paths.plan, JSON.stringify(result.plan, null, 2) + '\n');
    const outputs = actionOutputs(entry, result, this.paths);
    await this.output.appendFile(outputCommands(outputs), 'utf8');
    if (this.summary) await this.summary.appendFile(actionSummary(result), 'utf8');
    return outputs;
  }
  async close(): Promise<void> { await this.output.close(); await this.summary?.close(); }
}
