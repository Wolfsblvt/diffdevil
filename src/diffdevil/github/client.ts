...
export class GitHubClient {
  ...
  constructor(options: GitHubClientOptions = {}) {
    let url: URL;
    try { url = new URL(options.apiUrl ?? 'https://api.github.com/'); }
    catch { fail('E_CONFIG', 'GitHub API URL must be an absolute HTTPS URL.', 'config'); }
    if (url.protocol !== 'https:' || url.username || url.password || url.search || url.hash) fail('E_CONFIG', 'GitHub API URL must use HTTPS without credentials, query or fragment.', 'config');
    this.apiUrl = url.href.replace(/\/$/, '') + '/';
    this.#token = options.token;
    if (this.#token !== undefined && /[\r\n]/u.test(this.#token)) fail('E_CONFIG', 'GitHub token contains invalid header characters.', 'config');
    ...
    for (const [name, value] of [['timeoutMs', this.#timeout], ['readRetries', this.#retries], ['responseBytes', this.responseBytes]] as const) {
      if (!Number.isSafeInteger(value) || value < (name === 'readRetries' ? 0 : 1)) fail('E_CONFIG', `${name} must be a valid nonnegative safe integer (positive for sizes and timeouts).`, 'config');
    }
  ...
  url(route: string): URL {
    if (!route.startsWith('/') || route.startsWith('//') || route.includes('\\') || /[\r\n#]/u.test(route)) fail('E_GITHUB_ROUTE', 'Expected a relative GitHub API endpoint.', 'source');
    const result = new URL(route.slice(1), this.apiUrl);
    if (!result.href.startsWith(this.apiUrl)) fail('E_GITHUB_ROUTE', 'GitHub endpoint escaped the configured API root.', 'source');
    return result;
  }
  async request(route: string, options: RequestOptions = {}): Promise<GitHubResponse> {
    const url = this.url(route), method = options.method ?? 'GET', phase = options.phase ?? 'source';
    const headers: Record<string, string> = { accept: options.accept ?? 'application/vnd.github+json',
    ...
    for (let attempt = 0; ; attempt++) {
      ...
      try {
        ...
        if (method === 'GET' && attempt < this.#retries && !this.#signal?.aborted) { await this.#sleep(1000 * 2 ** attempt); continue; }
      ...
      try {
        if (reader) for (;;) {
          ...
          if (bytes > this.responseBytes) { await reader.cancel(); fail('E_LIMIT', 'GitHub response exceeds the selected response budget.', phase); }
      ...
      let text: string;
      try { text = new TextDecoder('utf-8', { fatal: true, ignoreBOM: true }).decode(joined); }
  ...
  async json(route: string, options: RequestOptions = {}): Promise<unknown> {
    ...
    catch { return fail('E_GITHUB_RESPONSE', 'GitHub returned malformed JSON.', options.phase ?? 'source'); }
  }
  /** Follow only same-endpoint, same-origin pagination. Never treat a provider URL as authority. */
  async list(route: string, options: { maximumItems?: number } = {}): Promise<{ items: unknown[]; truncated: boolean }> {
    const first = this.url(route), visited = new Set<string>(), items: unknown[] = [];
    let next: URL | undefined = first, bytes = 0;
    while (next) {
      if (visited.has(next.href)) fail('E_GITHUB_PAGINATION', 'GitHub repeated a pagination link.', 'source');
      visited.add(next.href);
      const relative = '/' + next.href.slice(this.apiUrl.length);
      const page = await this.request(relative);
      bytes += page.bytes;
      if (bytes > this.responseBytes) fail('E_LIMIT', 'GitHub collection exceeds the selected response budget.', 'source');
      let rows: unknown;
      try { rows = JSON.parse(page.text); } catch { fail('E_GITHUB_RESPONSE', 'GitHub collection is not JSON.', 'source'); }
      if (!Array.isArray(rows)) fail('E_GITHUB_RESPONSE', 'Expected a GitHub collection array.', 'source');
      items.push(...rows);
      ...
      if (target) {
        let url: URL;
        try { url = new URL(target, first); } catch { fail('E_GITHUB_PAGINATION', 'GitHub pagination URL is invalid.', 'source'); }
        if (url.origin !== first.origin || url.pathname !== first.pathname || url.username || url.password || url.hash) fail('E_GITHUB_PAGINATION', 'GitHub pagination escaped the selected endpoint.', 'source');
        for (const [key, value] of first.searchParams) if (key !== 'page' && key !== 'per_page' && url.searchParams.get(key) !== value) fail('E_GITHUB_PAGINATION', 'GitHub pagination changed the selected query.', 'source');
        for (const [key] of url.searchParams) if (!first.searchParams.has(key) && key !== 'page' && key !== 'per_page') fail('E_GITHUB_PAGINATION', 'GitHub pagination added an unexpected query.', 'source');
        next = url;
      }
      if (options.maximumItems !== undefined && items.length >= options.maximumItems) return { items: items.slice(0, options.maximumItems), truncated: Boolean(next) || items.length > options.maximumItems };
    }
