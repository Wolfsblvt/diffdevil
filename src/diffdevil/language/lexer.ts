...
const keyword = (name: string, pattern: RegExp, categories: typeof ComparisonOperator[] = []) =>
  createToken({ name, pattern, longer_alt: Identifier, categories });
...
export const StringLiteral = createToken({
  ...
  pattern: { exec(text: string, offset: number) {
    ...
    return [text.slice(offset, i)] as RegExpExecArray;
  } },
});
const punctuation = (name: string, pattern: RegExp, categories: typeof UnaryOperator[] = []) => createToken({ name, pattern, categories });
...
export function stringValue(token: IToken, source: ExpressionSource): string {
  try { return decodeStringLiteral(token.image, sourceRange(source, token.startOffset, token.endOffset! + 1)); }
  catch { return fail('E_STRING', 'Invalid string literal or Unicode escape.', 'lex', sourceRange(source, token.startOffset, token.endOffset! + 1)); }
}
export function numericValue(token: IToken, source: ExpressionSource): { literalType: 'integer' | 'float'; value: number } {
  ...
  if (!Number.isFinite(value)) fail('E_NUMBER_LITERAL', 'Float literal must be finite.', 'lex', range);
  return { literalType, value };
}
/** Strict lexing. A returned vector has no lexical recovery and contains validated literals. */
export function lexSource(source: ExpressionSource, limits: EvaluationLimits = DEFAULT_LIMITS): IToken[] {
  ...
  if (result.tokens.length > limits.tokens) fail('E_LIMIT', 'Expression exceeds its token limit.', 'lex');
  for (const token of result.tokens) {
    token.startOffset += bom;
    if (token.endOffset !== undefined) token.endOffset += bom;
    if (bom && token.startLine === 1) token.startColumn! += bom;
    if (bom && token.endLine === 1) token.endColumn! += bom;
    if (tokenMatcher(token, NumberLiteral)) numericValue(token, source);
    if (tokenMatcher(token, StringLiteral)) enforceBytes(stringValue(token, source), limits.stringBytes, 'String literal', 'lex');
  }
  const error = result.errors[0];
  if (error) fail('E_LEX_CHARACTER', 'Unexpected character in expression.', 'lex', sourceRange(source, error.offset + bom, error.offset + bom + error.length));
