...
export function runCli(argv:readonly string[],defaultCwd=process.cwd(),host:CliHost={}):Promise<Result<Rendered>>{
  return captureAsync(async()=>{
    ...
    if(v.help) output={stdout:HELP,exitCode:0};
      ...
      const kinds:Record<string,string>={report:'schemas/report-v1.schema.json',config:'schemas/policy-v1.schema.json',query:'schemas/query-result-v1.schema.json',plan:'schemas/plan-v1.schema.json',language:'spec/detail/v1/profile.json',metrics:'spec/detail/v1/shortcuts.json'};
