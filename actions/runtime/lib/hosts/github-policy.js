...
import { compilePolicyText, loadLocalPolicy } from './policy.js';
import { readUtf8, readWorkspaceUtf8 } from './io.js';
import { DEFAULT_LIMITS } from '../limits.js';
async function defaultBranchCommit(client, repository) {
...
export async function loadHostedGitHubPolicy(client, target, selection, cwd) {
    if (selection.config !== undefined && selection.inline !== undefined)
        fail('E_CONFIG_CONFLICT', 'Select a policy file or inline policy, not both.', 'config');
    const source = selection.source ?? 'base';
    if (!['base', 'pinned', 'workspace'].includes(source))
        fail('E_CONFIG', 'Policy source must be base, pinned or workspace.', 'config');
    if (source !== 'pinned' && (selection.ref !== undefined || selection.repository !== undefined))
        fail('E_CONFIG_CONFLICT', 'Policy ref/repository require explicit pinned policy source.', 'config');
    if (source === 'pinned' && selection.ref === undefined)
        fail('E_CONFIG', 'Pinned policy needs a full commit SHA in policy-ref.', 'config');
    const options = {
    ...
    if (selection.config === undefined && selection.inline === undefined) {
        if (selection.source !== undefined || selection.ref !== undefined || selection.repository !== undefined)
            fail('E_CONFIG_CONFLICT', 'A policy source selector requires config or inline policy.', 'config');
    ...
    const policy = selection.inline === undefined
        ...
        : await compilePolicyText(selection.inline, 'inline:policy', options, relative => {
            if (relative.startsWith('/') || relative.includes('\\') || /^[A-Za-z][A-Za-z0-9+.-]*:/u.test(relative))
                fail('E_POLICY_SOURCE', 'Template file must be repository-relative.', 'config');
    ...
    return { policy, ...(source === 'base' && target.pullRequest !== undefined ? { expectedPolicyBase: ref } : {}),
        ...(source === 'base' && target.pullRequest === undefined ? { assertCurrent: async () => {
                if (await defaultBranchCommit(client, target.repository) !== ref)
                    fail('E_POLICY_STALE', 'Repository base moved after policy acquisition. Reload policy before definition synchronization.', 'apply');
            } } : {}) };
}
//# sourceMappingURL=github-policy.js.map