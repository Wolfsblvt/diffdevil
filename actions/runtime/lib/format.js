...
import { agentBand, agentMeasurement, agentRule, agentSemantics, agentSource, evidenceLabel, humanBand, humanIdentifier, humanMeasurement, humanRule, humanSignedMeasurement, humanSource, measurementReasonSuffix, padLabel, paintAdded, paintBrand, paintDeleted, paintPrimary, paintQuiet, paintStrong, reasonCodes, } from './presentation.js';
...
function reportMarkdown(report) {
    const rows = [
        ...
        ...Object.entries(report.totals.files).map(([key, value]) => [`Files ${key}`, displayMeasurement(value)]),
        ...Object.entries(report.metrics ?? {}).map(([key, value]) => [`Metric ${JSON.stringify(key)}`, displayMeasurement(value)]),
        ...Object.entries(report.bands ?? {}).map(([key, value]) => [`Band ${JSON.stringify(key)}`, value.status === 'resolved' ? JSON.stringify(value.id) : `unknown; candidates ${JSON.stringify(value.candidates)}`]),
        ...Object.entries(report.rules ?? {}).map(([key, value]) => [`Rule ${JSON.stringify(key)}`, humanRule(value)]),
    ];
    return '# diffdevil analysis\n\n| Fact | Value |\n| --- | --- |\n' + rows.map(row => '| ' + row.map(markdownEscape).join(' | ') + ' |').join('\n') + '\n';
}
function humanMetricEntries(report, full) {
    ...
    for (const entry of entries.slice(3))
        if (entry[1].status !== 'exact')
            selected.push(entry);
    return selected;
}
function humanMetricsLine(report, full) {
    ...
    if (remaining)
        values.push(`… ${remaining} more`);
    return values.join(' · ');
}
function humanBandsLine(report) {
...
function humanRulesLine(report, full) {
    const entries = Object.entries(report.rules ?? {});
    if (!entries.length)
        return undefined;
    if (full)
        return entries.map(([name, value]) => `${humanIdentifier(name)} ${humanRule(value)}`).join(' · ');
    ...
    if (unmatched)
        values.push(`${unmatched} unmatched`);
    return values.join(' · ');
}
function nonZeroOrNonExact(value) {
    return value !== undefined && (value.status !== 'exact' || value.value !== 0);
}
function humanFilesLine(report) {
    ...
    if (nonZeroOrNonExact(files.binary))
        values.push(`${humanMeasurement(files.binary, report.measurement.status)} binary`);
    if (nonZeroOrNonExact(files.unmeasurable))
        values.push(`${humanMeasurement(files.unmeasurable, report.measurement.status)} unmeasurable`);
    if (!report.fileSet.complete)
        values.push('file set incomplete');
    return values.join(' · ');
}
function humanReportDetails(report, color) {
    const lines = ['', paintStrong('Details', color)];
    lines.push(`  ${padLabel('Schema')} ${report.schemaVersion}`);
    if (report.reportId !== undefined)
        lines.push(`  ${padLabel('Report')} ${JSON.stringify(report.reportId)}`);
    if (report.policyId !== undefined)
        lines.push(`  ${padLabel('Policy')} ${JSON.stringify(report.policyId)}`);
    ...
    if (report.semantics.presets?.length)
        lines.push(`  ${padLabel('Presets')} ${report.semantics.presets.map(value => JSON.stringify(value)).join(' · ')}`);
    if (report.semantics.limits !== undefined)
        lines.push(`  ${padLabel('Limits')} ${JSON.stringify(report.semantics.limits)}`);
    if (report.measurement.reasons.length)
        lines.push(`  ${padLabel('Evidence')} ${reasonCodes(report.measurement.reasons).join(' · ')}`);
    ...
    if (fileEntries.length) {
        ...
        for (const [name, value] of fileEntries)
            lines.push(`  ${padLabel(humanIdentifier(name))} ${humanMeasurement(value, report.measurement.status)}${measurementReasonSuffix(value)}`);
    }
    const scopes = Object.entries(report.scopes ?? {});
    if (scopes.length) {
        lines.push('', paintStrong('Scopes', color));
        for (const [name, scope] of scopes)
            lines.push(`  ${humanIdentifier(name)} · ${humanMeasurement(scope.totals.lines.changed, scope.fileSet.total.status)} changed · ${humanMeasurement(scope.totals.files.included ?? scope.fileSet.total, scope.fileSet.total.status)} files · ${scope.fileSet.complete ? 'complete' : 'incomplete'}`);
    }
    return lines;
}
function reportHuman(report, options) {
    ...
    if (report.measurement.reasons.length)
        lines.push(`${paintQuiet('Evidence', color)}   ${reasonCodes(report.measurement.reasons).join(' · ')}`);
    lines.push('');
    ...
    if (bands || metrics || rules) {
        ...
        if (bands)
            lines.push(`  ${padLabel(Object.keys(report.bands ?? {}).length === 1 ? 'Band' : 'Bands')}${bands}`);
        if (metrics)
            lines.push(`  ${padLabel('Metrics')}${metrics}`);
        if (rules)
            lines.push(`  ${padLabel(Object.keys(report.rules ?? {}).length === 1 ? 'Rule' : 'Rules')}${rules}`);
    }
    if (full)
        lines.push(...humanReportDetails(report, color));
    return lines.join('\n') + '\n';
}
function reportAgent(report) {
    ...
    for (const [name, scope] of Object.entries(report.scopes ?? {}))
        lines.push(`scope id=${JSON.stringify(name)} file_set=${scope.fileSet.complete ? 'complete' : 'incomplete'} files=${agentMeasurement(scope.fileSet.total)} changed=${agentMeasurement(scope.totals.lines.changed)} raw_churn=${agentMeasurement(scope.totals.raw.churn)}`);
    if (report.metrics && Object.keys(report.metrics).length)
        lines.push(`metrics ${Object.entries(report.metrics).map(([name, value]) => `${JSON.stringify(name)}=${agentMeasurement(value)}`).join(' ')}`);
    if (report.bands && Object.keys(report.bands).length)
        lines.push(`bands ${Object.entries(report.bands).map(([name, value]) => `${JSON.stringify(name)}=${agentBand(value)}`).join(' ')}`);
    for (const [name, value] of Object.entries(report.rules ?? {}))
        lines.push(`rule ${agentRule(name, value)}`);
    return lines.join('\n') + '\n';
}
export function formatReport(report, format = 'human', options = {}) {
...
export function formatReportWithOptions(report, format = 'human', options = {}) {
    return capture(() => {
        let stdout;
        if (format === 'json')
            ...
            stdout = reportMarkdown(report);
        else if (format === 'human')
            stdout = reportHuman(report, options);
        else if (format === 'agent')
            stdout = reportAgent(report);
        else
