...
import { Budget, enforceBytes } from '../limits.js';
...
import { programState } from './compile.js';
import { assertEnvironment, isReportEnvironment } from './environment.js';
import { isOptional, present } from './types.js';
...
function logical(operator, left, right) {
    ...
    if (left.status === 'resolved' && left.value === absorbing || right.status === 'resolved' && right.value === absorbing)
        return { status: 'resolved', value: absorbing };
...
function compare(operator, left, right, budget) {
    if (left.kind === 'number' && right.kind === 'number')
        return boolValue(numericComparison(operator, left, right));
    if (left.kind === 'null' && right.kind === 'null')
        return boolValue(operator === '==');
    const order = scalarOrder(left, right, budget);
    if (order === undefined)
        return boolValue(unknownDecision(mergeReasons(valueReasons(left), valueReasons(right))));
...
function nonnegativeSelector(node, source, env) {
    if (node?.kind === 'lambda')
        node = node.body;
    if (!node)
        return false;
    if (node.kind === 'literal' && typeof node.value === 'number')
        return node.value >= 0;
    if (!isReportEnvironment(env))
        return false;
    const normalizedFiles = (value) => {
        if (value.kind === 'identifier')
            return value.binding === 'root' && value.name === 'files';
...
function stringOperand(value) {
    if (value.kind !== 'string')
        fail('E_TYPE', 'Expected a present string during evaluation.');
    return value.value;
}
/** Interpret a bound program. Source text, dynamic properties and host callbacks are never executed. */
export function evaluateExpression(program, environment, options = {}) {
    return capture(() => {
        const state = programState(program);
        assertEnvironment(environment);
        if (program.schemaId !== environment.schemaId)
            fail('E_ENVIRONMENT_SCHEMA', 'The compiled program requires a different environment schema.');
        const budget = new Budget(options.limits ?? state.limits);
        const visit = (node, locals) => {
            budget.charge(1, node.span);
            try {
                return promote(run(node, locals), node.type);
            }
            catch (error) {
                if (error instanceof DiffdevilError && !error.diagnostic.range)
                    throw new DiffdevilError({ ...error.diagnostic, range: node.span, precision: 'character' });
                throw error;
            }
        };
        const run = (node, locals) => {
            const child = (input) => visit(input, locals);
            switch (node.kind) {
                case 'literal':
                    if (node.value === null)
                        return { kind: 'null' };
                    if (typeof node.value === 'number')
                        return numberValue({ status: 'exact', value: node.value }, node.type);
                ...
                case 'member': {
                    ...
                    if (object.kind === 'unknown')
                        return unknownValue(node.type, object.reasons);
                    if (object.kind !== 'record')
                        fail('E_TYPE', 'Only present records support member access.');
                ...
                case 'unary': {
                    ...
                    if (node.operator === '+')
                        return operand;
                    if (node.operator === '-')
                        return negate(asNumber(operand));
                    const d = asDecision(operand);
                    return boolValue(d.status === 'resolved' ? !d.value : d);
                }
                case 'binary': {
                    const left = child(node.left), op = node.operator;
                    if (op === '&&' || op === '||') {
                        const l = asDecision(left);
                        if (l.status === 'resolved' && l.value === (op === '||'))
                            return left;
                    ...
                    if (op === 'in')
                        return boolValue(quantifyCollection(asCollection(right), value => compare('==', left, value, budget), 'any', budget));
                ...
                case 'call': {
                    ...
                    const selector = (value) => {
                        if (selectorNode?.kind !== 'lambda')
                            fail('E_INTERNAL', 'Expected a bound collection selector.');
                        const nested = new Map(locals);
                        nested.set(selectorNode.parameter, value);
                    ...
                    switch (name) {
                        ...
                        case 'requireExact':
                            if (asNumber(a).measurement.status !== 'exact')
                                fail('E_EXACT_REQUIRED', 'This operation requires an exact numeric value.');
                            return a;
                        case 'lowerBound':
                        case 'upperBound': {
                            const number = asNumber(a), measurement = number.measurement;
                            if (measurement.status === 'exact' || measurement.status === 'unmeasurable')
                                return a;
                            const bound = bounds(measurement)[name === 'lowerBound' ? 0 : 1];
                            if (!Number.isFinite(bound))
                                return numberValue({ status: 'unknown', reasons: [{ code: 'BOUND_UNAVAILABLE' }] }, number.numericType);
                            budget.note({ code: 'BOUND_PROJECTION' });
                            return numberValue({ status: 'exact', value: bound }, number.numericType);
                        }
                        case 'isMissing':
                        case 'isNull':
                            if (a.kind === 'unknown' && isOptional(a.type))
                                return boolValue(unknownDecision(a.reasons));
                            return boolValue(a.kind === (name === 'isMissing' ? 'missing' : 'null'));
                        case 'requirePresent':
                            if (a.kind === 'missing' || a.kind === 'null' || a.kind === 'unknown' && isOptional(a.type))
                                fail('E_PRESENT_REQUIRED', 'This operation requires proven non-null presence.');
                            return a;
                        case 'orElse':
                            if (a.kind === 'missing' || a.kind === 'null')
                                return b();
                            if (a.kind === 'unknown' && isOptional(a.type))
                                return unknownValue(node.type, a.reasons);
                            return a;
                        case 'abs':
                        case 'floor':
                        case 'ceil': return numericHelper(name, asNumber(a));
                        ...
                        case 'any':
                        case 'all': return boolValue(quantifyCollection(asCollection(a), selector, name, budget, literalBoolean(selectorNode)));
                        ...
                        case 'length':
                            if (a.kind === 'unknown')
                                return unknownValue('integer', a.reasons);
                            budget.charge(stringOperand(a).length);
                            return integer(scalarCharacters(stringOperand(a), 'E_TYPE').length);
                        case 'startsWith':
                        case 'endsWith':
                        case 'contains':
                        case 'glob': {
                            const right = b();
                            if (a.kind === 'unknown' || right.kind === 'unknown')
                                return boolValue(unknownDecision(mergeReasons(valueReasons(a), valueReasons(right))));
                            const text = stringOperand(a), pattern = stringOperand(right);
                            budget.charge(text.length + pattern.length);
                        ...
                        case 'pathMatches': {
                            ...
                            if (a.kind === 'unknown' || patternValue.kind === 'unknown')
                                return boolValue(unknownDecision(mergeReasons(valueReasons(a), valueReasons(patternValue))));
                            if (a.kind !== 'record')
                                fail('E_TYPE', 'pathMatches requires a file record.');
                            const path = a.fields.path, oldPath = a.fields.oldPath, pattern = stringOperand(patternValue);
                            if (path.kind === 'unknown')
                                return boolValue(unknownDecision(path.reasons));
                            const current = stringOperand(path);
                            if (glob(current, pattern, budget))
                                return boolValue(true);
                            if (oldPath?.kind === 'unknown')
                                return boolValue(unknownDecision(oldPath.reasons));
