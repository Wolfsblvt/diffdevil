...
export async function loadActionPolicy(context, client) {
    const { inputs, mode } = context;
    const presets = inputs.preset === undefined ? undefined : choice(inputs.preset, ['size@1', 'none'], 'size@1', 'preset') === 'none' ? [] : ['size@1'];
    ...
    if (full && generatedRule.some(name => inputs[name] !== undefined))
        fail('E_CONFIG_CONFLICT', 'A full policy cannot be combined with generated rule/effect shorthand. Declare the rule in policy; metric/formula, scope and path may select read-only output.', 'config');
    const paths = {};
    for (const [flag, field] of [['exclude', 'exclude'], ['include-only', 'includeOnly'], ['force-include', 'forceInclude']]) {
        if (inputs[flag] !== undefined)
            paths[field] = inputs[flag].split(/\r?\n/u).map(line => line.trim()).filter(Boolean);
        if (inputs[`${flag}-mode`] !== undefined)
            paths[`${field}Mode`] = choice(inputs[`${flag}-mode`], ['append', 'replace'], 'append', `${flag}-mode`);
    }
    if (!full && Object.keys(shorthand).length) {
        if (['policy-source', 'policy-ref', 'policy-repository'].some(name => inputs[name] !== undefined))
            fail('E_CONFIG_CONFLICT', 'Policy source selectors require config or policy.', 'config');
    ...
    const selection = {
        ...
        ...(presets === undefined ? {} : { presets }), paths: paths,
    };
