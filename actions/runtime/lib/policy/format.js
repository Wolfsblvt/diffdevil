...
import { agentRule, agentSemantics, agentSource, humanIdentifier, humanRule, padLabel, paintBrand, paintQuiet, paintStrong, reasonCodes, } from '../presentation.js';
function planTarget(plan) { return `${plan.target.repository}#${plan.target.pullRequest}`; }
...
function agentOperation(operation) {
    switch (operation.kind) {
        case 'label.ensure':
...
function planMarkdown(plan) {
    ...
    if (!plan.operations.length)
        lines.push('No desired operations.');
    else
        for (const operation of plan.operations)
            lines.push(`- ${markdownCode(operation.kind)}: ${markdownCode(operationSubject(operation))}`);
    if (plan.held.length) {
        lines.push('', '## Held rules', '');
        for (const held of plan.held)
            lines.push(`- ${markdownCode(held.rule)}: ${reasonCodes(held.reasons).join(', ')}`);
    }
    lines.push('', '> No provider effects were applied.');
    return lines.join('\n') + '\n';
}
function planHumanDetails(plan, color) {
    ...
    if (plan.preconditions) {
        ...
        if (values.length)
            lines.push(`  ${padLabel('Requires')}${values.join(' · ')}`);
    }
    if (plan.operations.length) {
        lines.push('', paintStrong('Operations', color));
        for (const operation of plan.operations)
            lines.push(`  ${padLabel(operation.kind)}${operationSubject(operation)}`);
    }
    if (Object.keys(plan.rules).length) {
        lines.push('', paintStrong('Rules', color));
        for (const [name, value] of Object.entries(plan.rules))
            lines.push(`  ${humanIdentifier(name)} · ${humanRule(value)}`);
    }
    return lines;
}
function planHuman(plan, options) {
    ...
    for (const operation of plan.operations)
        if (operation.kind === 'label.add')
            lines.push(`${padLabel('Add')}${paintStrong(operation.name, color)} · rule ${humanIdentifier(operation.rule)}`);
    for (const operation of plan.operations)
        if (operation.kind === 'label.remove')
            lines.push(`${padLabel('Remove')}${paintStrong(operation.name, color)} · rule ${humanIdentifier(operation.rule)}`);
    for (const operation of plan.operations)
        if (operation.kind === 'comment.reconcile')
            lines.push(`${padLabel('Comment')}${humanIdentifier(operation.rule)} · ${operation.mode} · ${operation.trigger}`);
    ...
    if (ensure)
        lines.push(`${padLabel('Ensure')}${ensure} label definition${ensure === 1 ? '' : 's'}`);
    if (sync)
        lines.push(`${padLabel('Sync')}${sync} label definition${sync === 1 ? '' : 's'}`);
    if (!plan.operations.length)
        lines.push(`${padLabel('Proposed')}no desired operations`);
    if (plan.held.length)
        lines.push(`${padLabel('Held')}${plan.held.map(held => `${humanIdentifier(held.rule)} (${reasonCodes(held.reasons).join(', ')})`).join(' · ')}`);
    lines.push(paintQuiet(`${padLabel('Readback')}not observed`, color));
    if (full)
        lines.push(...planHumanDetails(plan, color));
    return lines.join('\n') + '\n';
}
function planAgent(plan) {
    const lines = [
    ...
    if (plan.preconditions) {
        ...
        if (values.length)
            lines.push(`preconditions ${values.join(' ')}`);
    }
    for (const [name, value] of Object.entries(plan.rules))
        lines.push(`rule ${agentRule(name, value)}`);
    for (const operation of plan.operations)
        lines.push(`effect ${agentOperation(operation)}`);
    if (plan.held.length)
        for (const held of plan.held)
            lines.push(`held rule=${JSON.stringify(held.rule)} reasons=${JSON.stringify(held.reasons)}`);
    else
        lines.push('held count=0');
    lines.push('readback observed=false');
    return lines.join('\n') + '\n';
}
/** Human output explains desired effects without claiming they were applied. */
export function formatPlan(plan, format = 'json', options = {}) {
    return formatPlanWithOptions(plan, format, options);
}
/** Format one desired plan through a presentation-specific projection. */
export function formatPlanWithOptions(plan, format = 'json', options = {}) {
    return capture(() => {
        if (format === 'json')
            return { stdout: JSON.stringify(plan, null, 2) + '\n', exitCode: 0 };
        if (format === 'human')
            return { stdout: planHuman(plan, options), exitCode: 0 };
        if (format === 'agent')
            return { stdout: planAgent(plan), exitCode: 0 };
        if (format === 'markdown')
            return { stdout: planMarkdown(plan), exitCode: 0 };
