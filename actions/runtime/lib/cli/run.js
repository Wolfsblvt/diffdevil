...
async function readExpression(invocation, cwd) {
...
async function source(invocation, cwd, selectedPolicy, host) {
    ...
    if (v.pr && v.repo) {
        ...
        report = unwrap(readReport(await readUtf8(resolve(cwd, String(v.report)))));
    else if (v['diff-file'] || v.stdin)
        report = unwrap(analyzeDiff(v.stdin ? await stdinText() : await readUtf8(resolve(cwd, String(v['diff-file'])))));
    else
        report = unwrap(await analyzeGit({ cwd, ...(v.staged ? { staged: true } : {}), ...(v.base ? { base: String(v.base), head: String(v.head) } : {}), ...(v.comparison ? { comparison: String(v.comparison) } : {}) }));
    if (!selectedPolicy && (v.exclude || v['include-only'] || v['force-include']))
        report = withPathPolicy(report, { ...(v.exclude ? { exclude: v.exclude } : {}), ...(v['include-only'] ? { includeOnly: v['include-only'] } : {}), ...(v['force-include'] ? { forceInclude: v['force-include'] } : {}) });
    return report;
}
/** Core invocation returns data; only the thin main entry point writes stdout. */
export function runCli(argv, defaultCwd = process.cwd(), host = {}) {
    return captureAsync(async () => {
        const invocation = parseInvocation(argv), v = invocation.values, cwd = resolve(defaultCwd, String(v.cwd ?? '.'));
        let output;
        if (v.help)
            output = { stdout: helpText(), exitCode: 0 };
        else if (v.version) {
            const metadata = JSON.parse(await readUtf8(fileURLToPath(new URL('../../../package.json', import.meta.url))));
            const version = metadata && typeof metadata === 'object' && 'version' in metadata ? metadata.version : undefined;
            if (typeof version !== 'string' || !version || /[\r\n\0]/u.test(version))
                fail('E_PACKAGE', 'Installed package metadata has no valid version.', 'source');
            output = { stdout: version + '\n', exitCode: 0 };
        }
        else if (invocation.command === 'apply' || invocation.command.startsWith('labels '))
            output = await runEffectCommand(invocation, cwd, githubClientForHost(host));
        else if (invocation.command === 'schema') {
            const kinds = { report: 'src/diffdevil/contracts/schemas/report-v1.schema.json', config: 'src/diffdevil/contracts/schemas/policy-v1.schema.json', query: 'src/diffdevil/contracts/schemas/query-result-v1.schema.json', plan: 'src/diffdevil/contracts/schemas/plan-v1.schema.json', language: 'src/diffdevil/contracts/detail/v1/profile.json', metrics: 'src/diffdevil/contracts/detail/v1/shortcuts.json' };
            const path = kinds[String(v.kind ?? 'report')];
            if (!path)
                fail('E_USAGE', 'Unknown schema kind.', 'config');
            ...
            if (!['analyze', 'query', 'check', 'plan', 'validate', 'explain'].includes(invocation.command))
                fail('E_USAGE', `Command ${invocation.command} is not implemented. See --help.`, 'config');
            const selected = await loadPolicy(invocation, cwd);
            if (invocation.command === 'validate' || invocation.command === 'explain') {
                if (!selected)
                    fail('E_REPORT_CONTEXT', 'Select a policy to validate or explain.', 'config');
                const view = explainPolicy(selected.policy);
                if (invocation.command === 'explain') {
                    if (v.format && v.format !== 'json' && v.format !== 'yaml')
                        fail('E_FORMAT', 'Policy explanations support JSON or YAML.', 'format');
                    output = { stdout: v.format === 'yaml' ? stringifyYaml(view.document) : JSON.stringify(view, null, 2) + '\n', exitCode: 0 };
                }
                else {
                    if (v.format && !['json', 'human'].includes(String(v.format)))
                        fail('E_FORMAT', 'Validation supports human or JSON output.', 'format');
                ...
                if (invocation.command === 'plan') {
                    if (!selected)
                        fail('E_REPORT_CONTEXT', 'Planning needs an explicit policy or selected built-in defaults.', 'config');
                    ...
                    let evaluated;
                    if (selected) {
                        const selector = invocation.expressionSelector === 'name' ? { query: String(v.name) } : v.band ? { band: String(v.band) } : expression ?? invocation.shortcut;
                        ...
                        report = queried.report;
                        evaluated = queried.result;
                    }
                        ...
                        const band = report.bands?.[String(v.band)];
                        if (!band)
                            fail('E_REPORT_CONTEXT', 'The report has no selected band result.', 'bind');
                        ...
                        if (invocation.command === 'check')
                            requireType(compiled.resultType, 'boolean');
                        evaluated = unwrap(evaluateExpression(compiled, environment));
                    }
                    output = unwrap(formatQuery(evaluated, { command: invocation.command, ...(v.format ? { format: String(v.format) } : invocation.command === 'query' && v.files ? { format: 'lines' } : {}), report }));
                }
            }
        }
        if (v.output) {
...
export function formatDiagnostic(d, json = false) { return json ? JSON.stringify(d) + '\n' : `${d.code}: ${d.message}${d.range ? ` [${d.range.start}:${d.range.end}]` : ''}\n`; }
