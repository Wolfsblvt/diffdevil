...
export async function analyzeGit(options = {}) {
    return captureAsync(async () => {
        if ((options.base === undefined) !== (options.head === undefined))
            fail('E_SOURCE', 'Use --base and --head together.', 'source');
        if (options.staged && options.base !== undefined)
            fail('E_SOURCE', 'Staged and revision comparisons cannot be combined.', 'source');
        if (options.comparison !== undefined && options.base === undefined)
            fail('E_SOURCE', 'A comparison mode requires --base and --head.', 'source');
        const environment = { ...process.env, GIT_TERMINAL_PROMPT: '0', GIT_OPTIONAL_LOCKS: '0', GIT_NO_REPLACE_OBJECTS: '1', GIT_CONFIG_COUNT: '0', LC_ALL: 'C' };
        // An ambient Git context must not redirect an explicitly selected working directory.
        for (const name of ['GIT_DIR', 'GIT_WORK_TREE', 'GIT_INDEX_FILE', 'GIT_OBJECT_DIRECTORY', 'GIT_ALTERNATE_OBJECT_DIRECTORIES', 'GIT_CONFIG_PARAMETERS', 'GH_TOKEN', 'GITHUB_TOKEN', 'ACTIONS_RUNTIME_TOKEN', 'INPUT_GITHUB-TOKEN'])
            delete environment[name];
        for (const name of Object.keys(environment))
            if (name.startsWith('INPUT_'))
                delete environment[name];
        const controlledConfig = ['-c', 'core.fsmonitor=false'];
        const git = async (args, maximumBytes = options.maximumBytes ?? 64 * 1024 * 1024, emptyWhenAbsent = false) => {
            try {
                ...
                try {
                    ...
                    return fail('E_SOURCE', 'Git returned non-UTF-8 paths or diff text; no lossy decoding was performed.', 'source');
                }
            }
            catch (error) {
                const e = error;
                if (emptyWhenAbsent && e.code === 1)
                    return '';
                if (e.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER')
                    fail('E_LIMIT', `Git diff exceeds the ${maximumBytes}-byte acquisition limit.`, 'source');
        ...
        const revision = (ref) => git(['rev-parse', '--verify', '--end-of-options', `${ref}^{commit}`], 4096).then(value => value.trim());
        if ((await git(['ls-files', '--unmerged', '-z'])).length)
            fail('E_SOURCE', 'The index contains unmerged entries. Resolve the conflicts before analysis.', 'source');
        ...
        if (await revision(options.head ?? 'HEAD') !== head)
            fail('E_SOURCE', 'The selected head moved during analysis. Run analysis again against the current head.', 'source');
